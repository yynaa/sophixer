...
impl CubeDemo {
  pub fn call_loop(
    ...
    let MidiPhysicalState::Analog8(rx) = lcxl2.get_position_state(LCXL2Position::Slider(1))? else {
      unreachable!()
    };
    let MidiPhysicalState::Analog8(ry) = lcxl2.get_position_state(LCXL2Position::Slider(2))? else {
      unreachable!()
    };
    let MidiPhysicalState::Analog8(rz) = lcxl2.get_position_state(LCXL2Position::Slider(3))? else {
      unreachable!()
    };
